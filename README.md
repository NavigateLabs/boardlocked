...
It came from trying to improve chunklocked in certain aspects, focusing on roleplaying, smoother progression and roguelike-ish elements where a run can end up completely different from a previous one. Eventually I found that this simple rule fixed most of my problems with that mode. Since you can only make one action per chunk, your choice really matters!
